...
static u64 ParseHaversinePairs(buffer InputJSON, u64 MaxPairCount, haversine_pair *Pairs)
{
    ...
    if (PairsArray)
    {
        ...
        json_element *Element = PairsArray->FirstSubElement;
        while (Element && (PairCount + 4 <= MaxPairCount)) // Unroll 4 iterations at a time
        {
            haversine_pair *Pair1 = Pairs + PairCount++;
            haversine_pair *Pair2 = Pairs + PairCount++;
            haversine_pair *Pair3 = Pairs + PairCount++;
            haversine_pair *Pair4 = Pairs + PairCount++;

            Pair1->X0 = ConvertElementToF64(Element, CONSTANT_STRING("x0"));
            Pair1->Y0 = ConvertElementToF64(Element, CONSTANT_STRING("y0"));
            Pair1->X1 = ConvertElementToF64(Element, CONSTANT_STRING("x1"));
            Pair1->Y1 = ConvertElementToF64(Element, CONSTANT_STRING("y1"));
            Element = Element->NextSibling;

            if (Element)
            {
                Pair2->X0 = ConvertElementToF64(Element, CONSTANT_STRING("x0"));
                Pair2->Y0 = ConvertElementToF64(Element, CONSTANT_STRING("y0"));
                Pair2->X1 = ConvertElementToF64(Element, CONSTANT_STRING("x1"));
                Pair2->Y1 = ConvertElementToF64(Element, CONSTANT_STRING("y1"));
                Element = Element->NextSibling;
            }

            if (Element)
            {
                Pair3->X0 = ConvertElementToF64(Element, CONSTANT_STRING("x0"));
                Pair3->Y0 = ConvertElementToF64(Element, CONSTANT_STRING("y0"));
                Pair3->X1 = ConvertElementToF64(Element, CONSTANT_STRING("x1"));
                Pair3->Y1 = ConvertElementToF64(Element, CONSTANT_STRING("y1"));
                Element = Element->NextSibling;
            }

            if (Element)
            {
                Pair4->X0 = ConvertElementToF64(Element, CONSTANT_STRING("x0"));
                Pair4->Y0 = ConvertElementToF64(Element, CONSTANT_STRING("y0"));
                Pair4->X1 = ConvertElementToF64(Element, CONSTANT_STRING("x1"));
                Pair4->Y1 = ConvertElementToF64(Element, CONSTANT_STRING("y1"));
                Element = Element->NextSibling;
            }
        }

        // Handle remaining pairs if MaxPairCount or remaining elements are not a multiple of 4
        while (Element && (PairCount < MaxPairCount))
        {
